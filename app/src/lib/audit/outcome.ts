...
export const REFUSED_EVENT_TYPES = [
   ...
   *
   * `template.import_refused` belongs here more than any of the others do: a refused import leaves
   * nothing behind anywhere else in the product — no Bot, no skill, no ledger row — so this is the
   * only view in which the attempt can be counted at all, and counting is the point. One refused
   * paste is somebody's typo. Forty in an afternoon, each turned away for a different reason, is
   * somebody mapping the edges of the parser, and that is only ever visible to a reader who can
   * list them together.
   *
   * `template.capability_declined` is an administrator turning an ask down, which is a refusal by a
   * person rather than by a rule. `template.capability_requested` is deliberately NOT here: nothing
   * was forbidden — configuration travels and capability does not, so an ask that lands unmet is
   * the designed behaviour — and padding this family with the designed outcome would teach a reader
   * to discount the refusals that are real.
   */
